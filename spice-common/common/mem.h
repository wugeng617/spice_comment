...
typedef struct SpiceChunk { //内存块，有点类似iovec
    uint8_t *data; //地址
    uint32_t len; //长度
} SpiceChunk;
...
//Spice数据块的特质标记
enum {
    SPICE_CHUNKS_FLAGS_UNSTABLE = (1<<0), //表示SpiceChunk里的地址指定的内存不稳定，
    								//可能会被释放，或者内容被修改，应该尽快copy出来
    								//spice里使用spice_chunks_linearize线性化来copy
    SPICE_CHUNKS_FLAGS_FREE = (1<<1) //SpiceChunks是否要调用free
};

typedef struct SpiceChunks { //Spice的数据一般用此结构表示
    uint32_t     data_size; //总大小
    uint32_t     num_chunks; //块数
    uint32_t     flags; //标记
    SpiceChunk   chunk[0]; //SPICECHUNK列表
} SpiceChunks;
