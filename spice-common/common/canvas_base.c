...
typedef struct CanvasBase {
    SpiceCanvas parent; //公共接口部分
    uint32_t color_shift; //颜色偏移
    uint32_t color_mask; //颜色掩码
    QuicData quic_data; //Quic解码器使用的环境

    uint32_t format; //画布的像素格式
    int width; //画布宽
    int height; //画布高
    pixman_region32_t canvas_region; //画布区域

#if defined(SW_CANVAS_CACHE) || defined(SW_CANVAS_IMAGE_CACHE)
    SpiceImageCache *bits_cache; //位图缓存
#endif
#ifdef SW_CANVAS_CACHE
    SpicePaletteCache *palette_cache; //调色板缓存
#endif
#ifdef WIN32
    HDC dc; //win32的画布操作句柄，linux和安卓直接操作pixman
#endif

    SpiceImageSurfaces *surfaces; //画布的图层

    LzData lz_data; //LZ解码器使用的解压环境
    GlzData glz_data; //GLZ解码器使用的解压环境
    SpiceJpegDecoder* jpeg; //jpeg解码器
    SpiceZlibDecoder* zlib; //zlib解码器

    void *usr_data; //私有数据
    spice_destroy_fn_t usr_data_destroy; //私有数据销毁函数
} CanvasBase;
...
//设置SpiceCanvas的data，SpiceCanvas只是接口，实际传入的一定是
//具体可实例化的结构，这里传入的是CanvasBase，设置data和data的销魂函数
