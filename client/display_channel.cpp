...
class VideoStream {
public:
	// 客户端对象、画布对象、显示通道、编码类型、数据方向、流宽、流高、源宽、源高、
	// 目标区域、裁剪类型、裁剪矩形数量、裁剪矩形集
...
private:
    RedClient& _client;//引用RedClient
    Canvas& _canvas; //引用画布
    DisplayChannel& _channel; //引用显示通道
    MJpegDecoder *_mjpeg_decoder; //MJPEG解码器
    int _stream_width; //流宽
    int _stream_height; //流高
    int _stride; //行长
    bool _top_down; //数据布局方向
    SpiceRect _dest; //目标区域
    QRegion _clip_region; //裁剪区域
    QRegion* _clip; //裁剪区域指针

    struct VideoFrame {//视频流帧对象
        uint32_t mm_time; //mm时间
        uint32_t compressed_data_size; //帧数据大小
        uint8_t* compressed_data; //帧数据（经过压缩）
    };

    uint32_t _frames_head; //帧头位置
    uint32_t _frames_tail; //帧尾位置
    uint32_t _kill_mark; //废帧掩码？
    VideoFrame _frames[MAX_VIDEO_FRAMES]; //帧缓冲区，最多三十帧

#ifdef WIN32
    HBITMAP _prev_bitmap; //windows下持有HBITMAP
    HDC _dc; //HDC
#endif
    uint8_t *_uncompressed_data; //解压后数据
    PixmapHeader _pixmap; //PIXMAP头
    uint64_t _update_mark; //更新掩码
    uint32_t _update_time; //更新时间

public:
    VideoStream* next; //视频流集合
};
...
VideoStream::VideoStream(RedClient& client, Canvas& canvas, DisplayChannel& channel,
    ...
    , _stride (stream_width * sizeof(uint32_t)) //要求4字节一个像素，这里_stride一定大于0
    , _top_down (top_down)
    , _dest (*dest)
    , _clip (NULL)
    , _frames_head (0)
    ...
    , next (NULL)
{
    ...
    if (codec_type != SPICE_VIDEO_CODEC_TYPE_MJPEG) {//要求MJPEG压缩格式
...
#else
		//分配解压后数据空间，这里内存空间向后增长
        _uncompressed_data = new uint8_t[_stride * stream_height]; 
#endif
        _pixmap.width = src_width; //设置pixmap的宽
        _pixmap.height = src_height; //设置pixmap的高
	...
	if (_top_down) {//数据行从上到下拼接，也有可能从下到上拼接
...
void VideoStream::release_all_bufs()
{
    for (int i = 0; i < MAX_VIDEO_FRAMES; i++) {
        delete[] _frames[i].compressed_data; //释放帧的压缩数据
    }
#ifdef WIN32
    if (_dc) {
...
#else
    delete[] _uncompressed_data; //释放解压后的数据
#endif
}
