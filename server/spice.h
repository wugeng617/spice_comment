...
typedef struct QXLInterface QXLInterface;//接口
typedef struct QXLInstance QXLInstance;//对象
typedef struct QXLState QXLState;//对象的实现
typedef struct QXLWorker QXLWorker;//Worker
typedef struct QXLDevMemSlot QXLDevMemSlot;//QXL设备内存槽
typedef struct QXLDevSurfaceCreate QXLDevSurfaceCreate;//QXL设备surface创建
...
struct QXLInterface {
    ...
    int (*get_command)(QXLInstance *qin, struct QXLCommandExt *cmd); //获取图形命令
    int (*req_cmd_notification)(QXLInstance *qin);//图形命令获取反馈函数
    void (*release_resource)(QXLInstance *qin, struct QXLReleaseInfoExt release_info);
    int (*get_cursor_command)(QXLInstance *qin, struct QXLCommandExt *cmd);//获取光标命令
    int (*req_cursor_notification)(QXLInstance *qin);//光标命令获取反馈函数
