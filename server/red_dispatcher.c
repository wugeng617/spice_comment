...
/* 显示通道连接时的处理函数，本函数不做实际处理，只是转发命令
   具体的命令执行，由Dispatcher的消息响应线程执行
*/
static void red_dispatcher_set_display_peer(
	RedChannel *channel, RedClient *client,
	...
	int num_caps, uint32_t *caps)
{
    ...
    spice_debug("%s", "");
    dispatcher = (RedDispatcher *)channel->data; //显示通道的data是RedDispatcher
...
void red_dispatcher_init(QXLInstance *qxl)
{
    ...
    red_dispatcher->base.update_area = qxl_worker_update_area; //废弃接口
