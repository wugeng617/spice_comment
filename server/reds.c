...
typedef struct RedLinkInfo {
    RedsStream *stream; //socket流
    SpiceLinkHeader link_header; //LINK头，spice会话建立时的link消息头部
    SpiceLinkMess *link_mess; //LINK消息体
    int mess_pos;
...
static void reds_info_new_channel(RedLinkInfo *link, int connection_id)
{
    ...
    /* add info + send event */
    if (reds_stream_is_ssl(link->stream)) { //不执行
...
static void reds_handle_main_link(RedLinkInfo *link)
{
    ...
    if (!reds->allow_multiple_clients) { //不允许多客户端，则断开所有spice会话
        reds_disconnect();
    }

    if (link_mess->connection_id == 0) { //第一次连接，connection_id == 0,
...
{
    if (link->link_mess->channel_type == SPICE_CHANNEL_MAIN) {//主通道连接
