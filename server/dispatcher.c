                             ...
                             void *payload)
{
	...
	// 根据消息特征中记录的消息大小，从参数中取对应长度的数据写到fd中
...
//在消息分发器中注册一个消息处理器
//消息处理器使用消息类型来索引，包含了消息分发后执行的句柄，消息的大小，ack特征
//不支持重复注册同一个消息类型的消息处理器
                                 ...
                                 size_t size, int ack)
{
    ...
    msg = &dispatcher->messages[message_type]; //取出消息处理器
    msg->handler = handler; //设置句柄
    msg->size = size;//设置消息大小
    msg->ack = ack;//消息释放有ack, 同步还是异步
    if (msg->size > dispatcher->payload_size) { //消息缓冲区过小，扩大缓冲区
        dispatcher->payload = realloc(dispatcher->payload, msg->size);
        dispatcher->payload_size = msg->size; //消息缓冲区大小
    }
}
                     ...
                     void *opaque)
{
	...
	//调用socketpair生成本地流式套接字，用于线程间通信，或者将
	//一个流程分成两个部分。一部分时请求，一部分是对请求进行响应
