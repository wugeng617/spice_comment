...
#define CACHE_HASH_KEY BITS_CACHE_HASH_KEY //使用id的后十位作为哈希值
...
static int FUNC_NAME(hit)(CACHE *cache, uint64_t id, int *lossy, DisplayChannelClient *dcc)
{
    ...
    item = cache->hash_table[CACHE_HASH_KEY(id)]; //得桶

    while (item) {
        if (item->id == id) { //找到哈希表项
            ring_remove(&item->lru_link);
            ring_add(&cache->lru, &item->lru_link);//更新lru
            spice_assert(dcc->common.id < MAX_CACHE_CLIENTS); //最多4个客户端
            item->sync[dcc->common.id] = serial; //设置dcc的同步序号
            cache->sync[dcc->common.id] = serial; //设置cache的同步序号
            *lossy = item->lossy; //返回缓存项的有损/无损标记
            break;
        }
        item = item->next; //查找下一个
    }
    pthread_mutex_unlock(&cache->lock);

    return !!item; //找到了返回真，否则返回假
}

// 设置的缓存id的压缩属性，成功返回true，失败返回false
...
/* CACHE添加，项PixmapCache中添加一项 */
static int FUNC_NAME(add)(CACHE *cache, uint64_t id, uint32_t size,
	int lossy, DisplayChannelClient *dcc)
{
    ...
    item = spice_new(NewCacheItem, 1); //创建哈希表项
    serial = red_channel_client_get_message_serial(&dcc->common.base); //DCC序号

    pthread_mutex_lock(&cache->lock);

    if (cache->generation != dcc->CACH_GENERATION) { //代数不一致，触发同步，不添加缓存
        if (!dcc->pending_pixmaps_sync) { //没同步，发送PIXMAP同步管道项触发同步
            red_channel_client_pipe_add_type(
                &dcc->common.base, PIPE_ITEM_TYPE_PIXMAP_SYNC);
            dcc->pending_pixmaps_sync = TRUE; //正在同步
        }
        pthread_mutex_unlock(&cache->lock);
        free(item);
        return FALSE;
	...
	//DCC的代数和缓存的代数一致
    cache->available -= size; //先占用空间
    while (cache->available < 0) { //空间不够了
        NewCacheItem *tail;
        NewCacheItem **now;

		// 如果整个缓存空间都空了，size超过缓存总大小，一个都加不进去
		// 此时tail为空，或者找到了，但是代数
        if (!(tail = (NewCacheItem *)ring_get_tail(&cache->lru)) ||
			tail->sync[dcc->common.id] == serial) 
		{
            cache->available += size; //释放缓存空间
            pthread_mutex_unlock(&cache->lock);
            free(item);//释放缓存项
            return FALSE;
        }
		//查找哈希表，必定可以找到，在lru里则必然在哈希表里
        ...
        ring_remove(&tail->lru_link); //从lru中删除
        cache->items--; //减计数
        cache->available += tail->size; //增加可用空间
        cache->sync[dcc->common.id] = serial; //更新序号
        //发送释放PIXMAP消息给客户端
