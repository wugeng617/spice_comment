...
/* 将QXLDataChunk的链表转换成RedDataChunk链表
   注意，这里的转换没有进行实际的数据块数据的复制，只是转换出了一个可访问的指针
   也就是说内存还是共享的，所以有了UNSTABLE标记，表标记内容是否稳定，如果是不稳定的
   内存，需要尽快线性化来copy出来
*/
                                      ...
                                      RedDataChunk *red, QXLDataChunk *qxl)
{
    ...
    red->data = qxl->data; //直接复制数据地址
    red->prev_chunk = NULL; //red是头结点

    while (qxl->next_chunk) {//迭代QXLDataChunk链表
        red_prev = red; //保存前结点
        red = spice_new(RedDataChunk, 1); //新建一个RedDataChunk，red表示当前结点
        //获取QXLDataChunk的HOST机地址
        memslot_id = get_memslot_id(slots, qxl->next_chunk);
        qxl = (QXLDataChunk *)get_virt(slots, qxl->next_chunk, sizeof(*qxl), group_id,
                                      &error);
        if (error) {//出错了返回0，貌似会有内存泄漏, 因为RedDataChunk的内存没释放
            return 0;
        }
        red->data_size = qxl->data_size;
        data_size += red->data_size;
        ...
        red->data = qxl->data;
        red->prev_chunk = red_prev; //设置前向指针
        red_prev->next_chunk = red; //更新前一个RedDataChunk的后向指针
    }

	//最后一个RedDataChunk的后向指针为空
    red->next_chunk = NULL;
    return data_size;
}

//从QXL物理地址中转换成RedDataChunk，addr是QXLDataChunk链表的物理地址
...
// RedDataChunk转换成SpiceChunks，不涉及到具体的数据copy
// 只复制了数据地址
                                 ...
                                 QXLPHYSICAL addr, uint32_t flags, int is_mask)
{
    ...
    case SPICE_IMAGE_TYPE_BITMAP://位图
        ...
        bitmap_size = red->u.bitmap.y * abs(red->u.bitmap.stride);//计算图像大小
        if (qxl_flags & QXL_BITMAP_DIRECT) {//数据存放在一整块内存中
            red->u.bitmap.data = red_get_image_data_flat(slots, group_id,
                                                         qxl->bitmap.data,
                                                         bitmap_size);
        } else {//Bitmap数据分块存放
        	//从QXLDataChunk转换出RedDataChunk双向链表，返回数据总大小
            size = red_get_data_chunks(slots, group_id,
                                       &chunks, qxl->bitmap.data);
            spice_assert(size == bitmap_size);
            if (size != bitmap_size) {//进行数据大小校验，
            	//如果red_get_data_chunks返回0可能内存泄漏
                goto error;
            }
			//将RedDataChunk转出成SpiceChunks
            red->u.bitmap.data = red_get_image_data_chunked(slots, group_id,
                                                            &chunks);
			//释放RedDataChunk
            red_put_data_chunks(&chunks);
        }
        if (qxl_flags & QXL_BITMAP_UNSTABLE) {//复制UNSTABLE标记到CHUNKS里
        	//为什么是CHUNKS？因为是bitmap.data这个数据块集里的内存地址不稳定，可能失效
            red->u.bitmap.data->flags |= SPICE_CHUNKS_FLAGS_UNSTABLE;
        }
        break;
    case SPICE_IMAGE_TYPE_SURFACE://SURFACE
        red->u.surface.surface_id = qxl->surface_image.surface_id;
        break;
    case SPICE_IMAGE_TYPE_QUIC://QUIC
        red->u.quic.data_size = qxl->quic.data_size; //复制数据块
        //为什么这里的qxl->quic.data不用转换成虚拟地址
        size = red_get_data_chunks_ptr(slots, group_id,
        	get_memslot_id(slots, addr),
        	&chunks, (QXLDataChunk *)qxl->quic.data);
		//答：因为QXL已经是虚拟地址，直接使用quic.data可以推测QUIC里
		//的QXLDataChunk直接跟跟随在QXLQUIKData的data_size之后，注意data[0]成员定义
		//而QXLBitmap的data是使用QXLPHYSICAL定义的指针，所以需要进行转换后才能得到
		//其qemu环境代码可访问的QXLDataChunk指针
        spice_assert(size == red->u.quic.data_size);
        if (size != red->u.quic.data_size) {
            goto error;
        }
		//RedDataChunk转SpiceChunks
...
void red_put_drawable(RedDrawable *red)
{
    ...
    switch (red->type) {//为什么没有COPY_BITS
    case QXL_DRAW_ALPHA_BLEND:
